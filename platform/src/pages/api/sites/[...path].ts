...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!session) return new Response(JSON.stringify({ success:false, error:{code:'UNAUTHORIZED',message:'로그인이 필요합니다.'} }), { status:401, headers:{'Content-Type':'application/json'} });
  try {
    const body = await request.json() as { siteName?: string; site_name?: string; domain: string };
    const siteName = body.siteName || body.site_name || '';
    const err = validateSiteCreation({ siteName, domain: body.domain });
    ...
    const r = await fetch(`${PLATFORM_API}/api/sites`, { method:'POST', headers:{'Content-Type':'application/json','X-User-Id':session.userId}, body:JSON.stringify({site_name:siteName,domain:body.domain}) });
    return new Response(await r.text(), { status:r.status, headers:{'Content-Type':'application/json'} });
  } catch { return new Response(JSON.stringify({ success:false, error:{code:'INTERNAL_ERROR',message:'서버 오류'} }), { status:500, headers:{'Content-Type':'application/json'} }); }
};
