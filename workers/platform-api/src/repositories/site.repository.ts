...
export class SiteRepository {
  ...
  async create(data: { user_id:string; site_name:string; domain:string; worker_id:string|null; d1_id:string|null; kv_id:string|null; wordpress_admin_username?:string; wordpress_admin_password_hash?:string; wordpress_admin_password_hint?:string; php_wasm_worker_name?:string; shard_count?:number; active_shard_key?:string; hosting_order_id?:string|null; plan_type?:string; worker_load_balancing?:string }): Promise<Site> {
    const id = generateId('site'); const ts = now();
    await this.db.prepare('INSERT INTO site_registry (id,user_id,worker_id,site_name,domain,d1_id,kv_id,wordpress_admin_username,wordpress_admin_password_hash,wordpress_admin_password_hint,php_wasm_worker_name,shard_count,active_shard_key,hosting_order_id,plan_type,worker_load_balancing,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').bind(id,data.user_id,data.worker_id,data.site_name,data.domain,data.d1_id,data.kv_id,data.wordpress_admin_username || null,data.wordpress_admin_password_hash || null,data.wordpress_admin_password_hint || null,data.php_wasm_worker_name || null,data.shard_count || 10,data.active_shard_key || 'database01.db',data.hosting_order_id || null,data.plan_type || 'lite',data.worker_load_balancing || 'single','provisioning',ts,ts).run();
